...
import { ChevronRight, ChevronLeft, Plus, Trash2, Check, Zap, AlignJustify, Layers, FileSpreadsheet, ClipboardPaste } from 'lucide-react';
...
function parseScoringText(text) {
  ...
  for (const line of lines) {
    ...
    const rangeMatch = line.match(/(\d+)\s*(?:st|nd|rd|th)?\s*[-–—to]+\s*(\d+)\s*(?:st|nd|rd|th)?[^:=\d]*[:\=\-]\s*(\d+)/i);
    if (rangeMatch) {
      for (let i = parseInt(rangeMatch[1]); i <= parseInt(rangeMatch[2]); i++)
        placementPoints.push({ position: i, points: parseInt(rangeMatch[3]) });
      continue;
    }
    const singleMatch = line.match(/(?:#)?(\d+)\s*(?:st|nd|rd|th)?\s*(?:place|position)?[^:=\d]*[:\=\-]\s*(\d+)/i);
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        ...
        {parsed?.placementPoints?.length > 0 && <button className="btn btn-primary btn-sm" onClick={doApply}><Check size={13} /> Apply Scoring</button>}
      </div>
      {parsed && (
        <div style={{ marginTop: 10, padding: '10px 14px', background: 'var(--bg-alt-row)', borderRadius: 8, fontSize: '0.8rem' }}>
          {parsed.killPointValue != null && <div style={{ color: 'var(--gold)', marginBottom: 6 }}>Kill Points: <strong>{parsed.killPointValue}</strong> per kill</div>}
          {parsed.placementPoints.length > 0 ? (
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {parsed.placementPoints.map(pp => (
                <span key={pp.position} style={{ padding: '3px 8px', background: 'var(--bg-header)', borderRadius: 5, color: 'var(--text-primary)' }}>#{pp.position}: <strong style={{ color: 'var(--gold)' }}>{pp.points}</strong></span>
              ))}
            </div>
...
export default function CreateTournamentPage() {
  ...
  const [placementPoints, setPlacementPoints] = useState(DEFAULT_PLACEMENT);
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto' }}>
      ...
      <div className="card">
        ...
        {step === 3 && (
          <div className="flex-col">
            <h2 className="card-title" style={{ marginBottom: 4 }}>Scoring Configuration</h2>

            <div className="form-field">
              <label className="form-label">Kill Point Value</label>
              <input className="form-input" type="number" min={0} step={0.5} value={killPointValue} onChange={e => setKillPointValue(e.target.value)} style={{ maxWidth: 160 }} />
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                {!showPaste && <button className="btn btn-secondary btn-sm" onClick={addPlacement}><Plus size={13} /> Add Row</button>}
              </div>
            </div>

            {showPaste ? (
              <PasteScoringPanel onApply={({ placementPoints: pp, killPointValue: kp }) => {
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '6px 10px', alignItems: 'center', maxWidth: 360 }}>
                ...
                {placementPoints.map((pp, i) => (
                  <Fragment key={`row-${i}`}>
                    <input type="number" className="form-input" value={pp.position} onChange={e => updatePlacement(i, 'position', e.target.value)} />
                    <input type="number" className="form-input" value={pp.points} onChange={e => updatePlacement(i, 'points', e.target.value)} />
